...
function dispatchDelete($table, $id){
	...
	switch($table){
		case 'obus':
		$res = DBObject::deleteEntry('obus', $id);
		break;
		case 'itinerary':
		$res = DBObject::deleteEntry('itinerary', $id, 'id_itin');
		break;
		case 'sequences':
		$res = DBObject::deleteEntry('sequences', $id, 'id_seq');
		break;
		case 'destination':
...
if(! is_numeric($_POST['id'])){
	...
	switch($_POST['table']){
		...
		break;
		case 'seq_stations':
		$res = DBObject::deleteEntry('seq_stations', $_POST['id'], 'id_ss');
		break;
		case 'pitstop':
		$res = Way::DeletePitstop($_POST['id']);		
		break;
	...
	/*	
		case 'obus_update':
		$obus = Obus::load( 8); //\LinkBox\Logger::log(serialize($obus) );
		$res = $obus->update(array('name'=>'sss'));		
		break;
		case 'chart_redraw_seq':
			$sequence_id = $_POST['id'];
			$seqstats = sequencesStations::getSeqStatNamesBySequenceID($sequence_id);
